...
enum protoCmd rx_request(struct protocol *req, int fd) {
  ...
  syslog(LOG_DEBUG,
         "DEBUG: Got command %s fd(%d) xml(%s), z(%d), x(%d), y(%d), mime(%s), "
         "options(%s)",
         cmdStr(req->cmd), fd, req->xmlname, req->z, req->x, req->y,
         req->mimetype, req->options);

  if ((req->cmd != cmdRender) && (req->cmd != cmdRenderPrio) &&
      (req->cmd != cmdRenderLow) && (req->cmd != cmdDirty) &&
      (req->cmd != cmdRenderBulk)) {
    syslog(LOG_WARNING,
           "WARNING: Ignoring unknown command %s fd(%d) xml(%s), z(%d), x(%d), "
           "y(%d)",
           cmdStr(req->cmd), fd, req->xmlname, req->z, req->x, req->y);
    return cmdNotDone;
  }
    ...
    if (num == -1)
      ...
      if (FD_ISSET(listen_fd, &rd)) {
        ...
        if (incoming < 0) {
          ...
          if (num_connections == MAX_CONNECTIONS) {
            ...
            syslog(LOG_DEBUG,
                   "DEBUG: Got incoming connection, fd %d, number %d\n",
      ...
      for (i = 0; num && (i < num_connections); i++) {
        ...
        if (FD_ISSET(fd, &rd)) {
          ...
          // TODO: to get highest performance we should loop here until we get
          // EAGAIN
          ...
          if (ret < 1) {
            ...
            num_connections--;
            syslog(LOG_DEBUG,
                   "DEBUG: Connection %d, fd %d closed, now %d left\n", i, fd,
                   num_connections);
...
void *stats_writeout_thread(void *arg) {
  ...
  while (1) {
    ...
    reqPrioQueueLength =
        request_queue_no_requests_queued(render_request_queue, cmdRenderPrio);
    reqQueueLength =
        request_queue_no_requests_queued(render_request_queue, cmdRender);
    reqLowQueueLength =
        request_queue_no_requests_queued(render_request_queue, cmdRenderLow);
    dirtQueueLength =
        request_queue_no_requests_queued(render_request_queue, cmdDirty);
    reqBulkQueueLength =
        request_queue_no_requests_queued(render_request_queue, cmdRenderBulk);
    ...
    if (statfile == NULL) {
      ...
      if (rename(tmpName, config.stats_filename)) {
        ...
        if (noFailedAttempts > 3) {
          syslog(LOG_ERR,
                 "ERROR: Failed repeatedly to overwrite stats, giving up");
          break;
        }
        continue;
      }
  ...
  if (sConfig->ipport > 0) {
    ...
    for (rp = result; rp != NULL; rp = rp->ai_next) {
      ...
      case AF_INET6:
        ...
        break;
      default:
        snprintf(ipstring, sizeof(ipstring), "address family %d",
                 rp->ai_family);
        break;
      }
      syslog(LOG_DEBUG,
 ...
 * the slave renderer. It then blocks and waits for the response with no
 * timeout. As it only sends one request at a time (there are as many
 * slave_thread threads as there are rendering threads on the slaves) nothing
 * gets queued on the slave and should get rendererd immediately. Thus overall,
 * requests should be nicely load balanced between all the rendering threads
 * available both locally and in the slaves.
 */
  ...
  while (1) {
    ...
    if (item) {
      ...
      while ((ret_size < sizeof(struct protocol)) && (retry > 0)) {
        ret_size =
            recv(pfd, resp + ret_size, (sizeof(struct protocol) - ret_size), 0);
...
int main(int argc, char **argv) {
  ...
  while (1) {
    ...
    case 'h':
      ...
      fprintf(stderr, "  -c, --config=CONFIG  set location of config file "
                      "(default /etc/renderd.conf)\n");
      fprintf(stderr, "  -s, --slave=CONFIG_NR set which render slave this is "
                      "(default 0)\n");
      exit(0);
    default:
  ...
  for (int section = 0; section < iniparser_getnsec(ini); section++) {
    ...
    if (strncmp(name, "renderd", 7) && strcmp(name, "mapnik")) {
      ...
      config_slaves[render_sec].socketname =
          iniparser_getstring(ini, buffer, (char *)RENDER_SOCKET);
      sprintf(buffer, "%s:iphostname", name);
      config_slaves[render_sec].iphostname =
          iniparser_getstring(ini, buffer, "");
      ...
      config_slaves[render_sec].num_threads =
          iniparser_getint(ini, buffer, NUM_THREADS);
      sprintf(buffer, "%s:tile_dir", name);
      config_slaves[render_sec].tile_dir =
          iniparser_getstring(ini, buffer, (char *)HASH_PATH);
      ...
      if (render_sec == active_slave) {
        ...
        config.mapnik_font_dir =
            iniparser_getstring(ini, "mapnik:font_dir", (char *)FONT_DIR);
        config.mapnik_font_dir_recurse =
            iniparser_getboolean(ini, "mapnik:font_dir_recurse", FONT_RECURSE);
      } else {
  ...
  render_init(config.mapnik_plugins_dir, config.mapnik_font_dir,
              config.mapnik_font_dir_recurse);
  ...
  if (active_slave == 0) {
    ...
    k = 0;
    slave_threads = (pthread_t *)malloc(sizeof(pthread_t) * noSlaveRenders);
